// Below lie some examples. Remember that it is always a good practice to use **dellist** when you 
...
// The **fill_list** function creates a list with the number you specify as the first parameter, with length that you specify as the second parameter. If you call fill_list on a non-empty list it will erase it and create a new one. 

// Finally if you want to give certain values to your list you can use **set_list** function.
...
// **CAREFUL** You must always initialize the array so that its last value is '\0' character in order for the function to work correctly.


// ###Example 3:Inserting to the list.
// The **insert** function inserts to the place specified by the fist parameter , the number specified in the second. Note that **insert** _can not_ insert to the end of a list unless it is empty. You will have to use append to do it. Here is an example:
