...
char *subsequence(char string[], int lower, int upper) {
  int len = strlen(string);
  if (upper > len || lower < 0 || lower > upper)
    return string;
...
int endswith(char string[], char substring[]) {
  ...
  if (!strcmp(correct_end, substring))
    return 1;
  return 0;
}
...
int startswith(char string[], char substring[]) {
  ...
  if (!strcmp(correct_start, substring))
    return 1;
  return 0;
}
...
char *lowercase(char string[]) {
  ...
  do {
    if (string[letter] > 64 && string[letter] < 91)
      newstring[letter] = string[letter] + 32;
  } while (letter++ < len);
  return newstring;
}
...
char *uppercase(char string[]) {
  ...
  strcpy(newstring, string);
  do {
    if (string[letter] > 96 && string[letter] < 123)
      newstring[letter] = string[letter] - 32;
  } while (letter++ < len);
  return newstring;
}
...
char *replace(char string[], char toreplace[], char replacer[]) {
  ...
  if ((index = contains(string, toreplace)) == -1)
    return string;

  char *newstring = malloc(sizeof(char) * final_length);
